class ddr4_invalid_rd_addr_sequence#(parameter ADDR_WIDTH=32, DATA_WIDTH=16)  extends uvm_sequence#(ddr4_seq_item#(ADDR_WIDTH, DATA_WIDTH));

`uvm_object_param_utils(ddr4_invalid_rd_addr_sequence#(ADDR_WIDTH, DATA_WIDTH))
ddr4_seq_item #(ADDR_WIDTH, DATA_WIDTH) ddr4_req;
ddr4_invalid_wr_addr_sequence#(ADDR_WIDTH, DATA_WIDTH) ddr4_invalid_wr_addr_seq;

bit [31:0] address;

function new(string name="ddr4_invalid_rd_addr_sequence");
super.new(name);
endfunction

virtual task body();
ddr4_req=ddr4_seq_item #(ADDR_WIDTH, DATA_WIDTH) ::type_id::create("ddr4_req");
 if(!uvm_config_db#(ddr4_invalid_wr_addr_sequence#(ADDR_WIDTH,DATA_WIDTH))::get(null,"", "ddr4_invalid_wr_addr_sequence", ddr4_invalid_wr_addr_seq))
 begin
...
address=ddr4_invalid_wr_addr_seq.wr_addr_q.pop_front();
